...
-   How data-first tools help with reproducible workflows and computational thinking
...
-   Tool suite
    ...
    -   Quarto Notebooks (Merging literate coding with Report rendering)
...
-   RStudio Projects and reproducibility
    ...
    -   **excise** `setwd()` and `rm(list = ls())`

-   Vectorized iteration ; environment variables ; pipes ; assignment

    -   pipes = `|>` and `%>%`

    -   Assignment variables: `<-` and `=` (not the same as equivalency `==`)

    -   Functional programming means customizing the functions and iterating without FOR loops
